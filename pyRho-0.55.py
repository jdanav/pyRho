...
def openXML():
    ...
    try:
        ...
        for widget in root.winfo_children()[1:]: widget.destroy()
        ...
        for frame in [main, f1S, f2S]:
            ...
            frame.label = ttk.Label(frame, text = '%s nodes and %s leaves in %s layers' % (len(n.nodes), len(n.leaves), len(n.layers)))
